...
def handle_get(handler: object, path: str) -> bool:
    ...
    if path == "/v1/status":
        handler._send_cached_json(
            "GET /v1/status",
            lambda: server_module._status_body(
    ...
    if path == "/v1/routing/status":
        handler._send_cached_diagnostics_json(
            ...
            lambda: server_module._routing_status_body(handler.server.config, handler.server.router),
        )
        return True
    if path == "/v1/routing/last-decision":
        handler._send_cached_diagnostics_json(
            "GET /v1/routing/last-decision",
            lambda: server_module._routing_last_decision_body(handler.server.config),
        )
        return True
    if path == "/v1/routing/test-failover":
        handler._send_cached_diagnostics_json(
            "GET /v1/routing/test-failover",
            lambda: server_module._routing_test_failover_body(handler.server.config, handler.server.router),
        )
        return True
    if path == "/v1/client-sources":
        handler._send_cached_diagnostics_json(
            "GET /v1/client-sources",
            lambda: server_module._client_sources_body(handler.server.config, handler.server.router),
        )
        return True
    if path == "/v1/routing-history":
        handler._send_cached_json(
            "GET /v1/routing-history",
            lambda: server_module._routing_history_body(handler.server.config),
        )
        return True
    if path == "/v1/provider-scores":
        handler._send_cached_json(
            "GET /v1/provider-scores",
            lambda: handler.server.diagnostics_service.provider_scores_body(),
        )
        return True
    if path == "/v1/provider-health":
        handler._send_cached_diagnostics_json(
            "GET /v1/provider-health",
            lambda: server_module._provider_health_body(handler.server.config, handler.server.router),
        )
        return True
    if path == "/v1/routing-memory/stats":
        handler._send_cached_diagnostics_json(
            "GET /v1/routing-memory/stats",
            lambda: server_module._routing_memory_stats_body(handler.server.config, handler.server.router),
        )
        return True
    if path == "/v1/routing-memory/recent":
        ...
        try:
            ...
            limit = 50
        handler._send_cached_diagnostics_json(
            f"GET /v1/routing-memory/recent?limit={limit}",
            lambda: server_module._routing_memory_recent_body(
                handler.server.config,
                handler.server.router,
                limit=limit,
            ),
    ...
    if path.startswith("/v1/routing-decision/"):
        ...
        handler._send_cached_diagnostics_json(
            f"GET /v1/routing-decision/{request_id}",
            lambda: server_module._routing_decision_by_id_body(
                handler.server.config,
                handler.server.router,
                request_id,
            ),
